...
// var rand = Math.floor(Math.random() * (the_data.length)) ; //this generates a random number between 0 and 100 in other words Math.floor(Math.random() * (max - min + 1)) + min; 
  //or Math.floor(Math.random()*(99-0+1)+0) 
...
//         var ans = all_data[next]["quiz"]["answer"]
//         var q = all_data[next]["quiz"]["question"]
     ...
     //  button =  '<div class="col-sm-5"><button type="button" id="show_next_single" onclick=\'play_random(1)\' class="btn btn-primary btn-block btn btn-outline-primary">Show random question</button></div>';
      ...
      // (sessionStorage.getItem("questions_not_shown"))  
...
*/
  ...
  //choose_index(direction,current_counter,counter_array)
   ...
   // var pre_button = '<button type="button" id="show_previous_single" onclick=\'choose_index("previous",'+current_counter+')\' class="btn btn-outline-primary"><i class="fa fa-arrow-left fa-1x" aria-hidden="true"></i></button>';
  
   
    // var ne_button =  '<button type="button" id="show_ne" onclick=\'choose_index("next",'+current_counter+')\' class="btn btn-outline-primary"><i class="fa fa-arrow-right fa-1x" aria-hidden="true"></i></button>';
     ...
     //var ne_button =  '<button type="button" id="show_next_single" onclick=\'play_random()\' class="btn btn-outline-primary"><i class="fa fa-arrow-right fa-1x" aria-hidden="true"></i></button>';
      var button = '<button type="button" id="show_next_single" onclick=\'play_random()\' class="btn btn-primary btn-block">Next</button>';
      //var button=""
      //var tough = '<div class="col-sm-5"><button type="button" id="show_tough" class="btn btn-secondary btn-block">Take difficult quiz</button></div>'
      var tough="";
      //var cbox_html = '<div class="container form-check"><div class="col-sm-12 skin"><label class="form-check-label"><input id="dif" class="form-check-input" type="checkbox" value="'+id+'">I found this question difficult</label></div></div>' 
      var cbox_html=""
      //var dif_qs = dif_ucis ? "<h5>Difficult Question Numbers</h5>"+dif_ucis : ""  
      ...
      //var markup ='<div class="container">'+'<div class="row">&nbsp;' +'</div>'  +'<div class="row">' +'<div class="col-sm-6">'+'<div class="flippable flip-container" ontouchstart="this.classList.toggle(\'hover\');">'+'<div id="flippable" class="flippable appcon ac-university">'+'<div class="front"> <span>UCIS Q#'+ucis_id+" "+q+'</span> <i class="fa fa-university"></i>'+'</div>'+'<div class="back">'+ans+'</div>'+'</div>'+'</div>' +'</div>'+button +'</div>'+'<div>'+pre_button+"&nbsp;"+ne_button;
       var text_q= "question"
      ...
      //var flip_html = '<div class="flippable flip-container" ontouchstart="this.classList.toggle(\'hover\');"><div id="flippable" class="flippable appcon ac-university"><div class="front">'+image+' <span>UCIS Q#'+ucis_id+" "+q+'</span>'+'</div><div class="back"><span>'+image+'</span><br>'+ans+'<br></div></div></div></div>'+ne_button +'</div><div>'
      
      //var markup ='<div class="container">'+'<div class="row">' +'</div>'  +'<div class="row">' +pre_button+'&nbsp;&nbsp;&nbsp;&nbsp;<div class="col-sm-4">'+cbox_html+flip_html+'<div class="row"><div class="col-sm-1"></div>'+audio_button+audio_answer_button+"</div><br>"+button+"<br>"+tough;
      var markup;
      //<select><option value="volvo">Volvo</option></select>
       ...
       //<buttonid="submit_state" type="button" class="btn btn-primary">Submit</button><br><br><h1>here</h1>'
       ...
       //https://app-engine-skills-abirqasem1.c9users.io/get_politicians?who=senators&state=AK
 
 
      if (show_dropdown_state){
      // $("#state").html(drop_html);
...
//         alert(msg+"  "+keys[index]["role_name"]);
 //        $.getJSON( 'https://app-engine-skills-abirqasem1.c9users.io/get_politicians?who="'+keys[index]["role_name"]+'&state="'+msg+'"', function( data ) {(data)})
...
//        alert("Data: " + data + "\nStatus: " + status);
 //   });
     ...
     //      $.ajax({url: 'https://app-engine-skills-abirqasem1.c9users.io/get_politicians?who="'+keys[index]["role_name"]+'&state="'+msg+'"', success: function(result){
  ...
  //  }});
     ...
     //    $("#ans_text").css({-moz-transform: "scaleX(-1)",-o-transform: "scaleX(-1)",-webkit-transform: "scaleX(-1)",transform:"scaleX(-1)",filter: "FlipH",-ms-filter: "FlipH"})

        $(this).toggleClass("flipme"); }); 
        $("#show_tough").click(function(){
         // var ids = sessionStorage.getItem ('difficult');
         // sessionStorage.setItem('not_done_all',ids); 
...
//{body: "#F95738",card:"#0D3B66",text:"#EE964B",div1:"F4D35E", div2: "#FAFOCA" }
/*
$("body").css({"background-color":palette1["white"]});
$(".back").css({"background-color":palette1["red"],"color":palette1["white"]});
$(".front").css({"background-color":palette1["red"],"color":palette1["white"]});
$(".back").css({"color":palette1["red"],"color":palette1["white"]});
$(".ac-university").css({"color":palette1["white"],"color":palette1["white"]});  
$('.color-fa').css({"color":palette1["white"]});
$('#pbar').css({"background-color":palette1["blue"]});
$('#pbar_inner').css({"background-color":palette1["red"]});
$('#pbar_text').css('color', 'white');
$('.btn').css({"background-color":palette1["blue"]});
$('.btn').css({"color":palette1["white"]});
*/
...
//99 left and 100 is also 1 left
   // document.getElementById('time_msg').innerHTML = num_q_shown+" questions in "+ hours+" hours "+minutes+" minutes "+seconds+" seconds. You have "+num_q_not_shown + ' left</b>'
  ...
  times++
   ...
   //if(num_q_not_shown===0){terminate();return;}
...
$("#pbar").hide();
/*  $('#pbar').css('background-color', 'white');
   $('#pbar_inner').css('background-color', 'white');
  $('#pbar_text').css('background-color', 'white');
   $('#pbar_text').css('color', 'white');*/
...
function typeAhead(availableTags) {
  ...
  var palette1 = {white: "white",red:"#BF0A30",white:"white",blue:"#002868", black: "black"}
